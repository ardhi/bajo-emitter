...
async function persistence (mod, args) {
  ...
  const { collExists, recordCreate } = this.app.bajoDb.helper
  const { isPlainObject } = this.app.bajo.helper._
...
async function collectEvents () {
  const me = this
  const { eachPlugins, runHook, importModule } = me.app.bajo.helper
  const { map, camelCase, merge, filter, groupBy, orderBy, isString } = me.app.bajo.helper._
  me.events = me.events ?? []
  await runHook('bajoEmitter:beforeCollectEvents')
  // collects
  me.log.trace('Collecting events...')
  await eachPlugins(async function ({ ns, dir, file }) {
    ...
    me.events.push(mod)
  }, { glob: 'event/**/*.js', baseNs: '/bajoEmitter' })
  // apply events
  await eachPlugins(async function ({ ns }) {
    for (const m of methods) {
      ...
      for (const i in items) {
        ...
        me.log.trace('- Collect event: %s:%s.%s (%d)', m, ns, i, mods.length)
  ...
  me.log.debug('Events collected: %d', me.events.length)
